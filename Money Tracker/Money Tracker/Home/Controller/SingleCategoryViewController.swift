...
protocol SingleCategoryViewControllerDelegate: AnyObject {
...
extension SingleCategoryViewController: UISearchBarDelegate {
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String) {
        ...
        if searchText != "" {
            searchedTransactions = transactions.filter({ (transactions) -> Bool in
                if let category = transactions.category?.localized(),
                   category.lowercased().contains(searchText.lowercased()) {
                    return true
                }
                if let title = transactions.title, title.lowercased().contains(searchText.lowercased()) {
                    return true
                }
                return false
            })
