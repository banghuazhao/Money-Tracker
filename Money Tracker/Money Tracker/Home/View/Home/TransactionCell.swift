    ...
    var searchText: String = "" {
        didSet {
            if let category = transaction?.category?.localized() {
                let attrString: NSMutableAttributedString = NSMutableAttributedString(string: category)

                let range: NSRange = (category as NSString).range(of: searchText, options: [NSString.CompareOptions.caseInsensitive])
            ...
            if let title = transaction?.title {
                ...
                let range: NSRange = (title as NSString).range(of: searchText, options: [NSString.CompareOptions.caseInsensitive])
