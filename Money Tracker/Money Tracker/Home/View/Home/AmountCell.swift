...
class AmountCell: UITableViewCell {
    ...
    private lazy var netWorthTitleLabel = UILabel().then { label in
        label.text = "Net Worth".localized()
    ...
    override init(style: UITableViewCell.CellStyle, reuseIdentifier: String?) {
        ...
        contentView.addSubview(netWorthTitleLabel)
        contentView.addSubview(netWorthLabel)
        contentView.addSubview(detailButton)
        contentView.addSubview(summaryStack)

        netWorthTitleLabel.snp.makeConstraints { make in
            make.top.equalToSuperview().offset(14)
            make.left.equalToSuperview().offset(16)
        }

        netWorthLabel.snp.makeConstraints { make in
            make.top.equalTo(netWorthTitleLabel.snp.bottom).offset(2)
            make.left.equalToSuperview().offset(16)
